...
import type { PullExportState } from './exportState';
...
export function markerToDomId(marker: MarkerId): string | null {
    switch (marker.type) {
        case 'issue-comment':
...
export function resolveMarkerElement(marker: MarkerId): HTMLElement | null {
...
export interface PullDropdownOptions extends DropdownBaseOptions {
    ...
    onPullPresetChange: (preset: PullPreset) => PullExportState;
    onPullAdvancedToggle: (option: keyof PullExportOptions, checked: boolean) => PullExportState;
}
...
export interface ActionsRunDropdownOptions extends DropdownBaseOptions {
    kind: 'actions-run';
    actionsRunPreset: ActionsRunPreset;
    onActionsRunPresetChange: (preset: ActionsRunPreset) => ActionsRunPreset;
}
...
const pullPresetLabels: Array<{ value: PullPreset; label: string }> = [
...
const actionsRunPresetLabels: Array<{ value: ActionsRunPreset; label: string }> = [
...
const pullOptionGroups: Array<{
    title: string;
    options: Array<{ key: keyof PullExportOptions; label: string }>;
}> = [
    {
        title: 'Content',
        options: [
...
export function createSettingsDropdown(opts: DropdownOptions): DropdownResult {
    ...
    if (opts.kind === 'pull') {
        ...
        const optionInputs: Partial<Record<keyof PullExportOptions, HTMLInputElement>> = {};
        const optionRows: Partial<Record<keyof PullExportOptions, HTMLLabelElement>> = {};
        ...
        const syncPullControls = (state: PullExportState): void => {
            ...
            Object.keys(optionInputs).forEach((key) => {
                const optionKey = key as keyof PullExportOptions;
        ...
        presetSelect.addEventListener('change', () => {
            const nextState = opts.onPullPresetChange(presetSelect.value as PullPreset);
            syncPullControls(nextState);
        ...
        const syncActionsRunControls = (preset: ActionsRunPreset): void => {
            presetSelect.value = preset;
        };

        presetSelect.addEventListener('change', () => {
            const nextPreset = opts.onActionsRunPresetChange(presetSelect.value as ActionsRunPreset);
