...
describe('PR preset rendering', () => {
  it('renders commit-log preset with commit sections only', () => {
    const markdown = prToMarkdown({
      ...
      reviewComments,
      reviews,
      options: resolvePreset('commit-log'),
    });
  ...
  it('renders review-comments-only preset with review comments only', () => {
    const markdown = prToMarkdown({
      ...
      reviewComments,
      reviews,
      options: resolvePreset('review-comments-only'),
    });
  ...
  it('honors timeline ordering when timeline mode is enabled', () => {
    const markdown = prToMarkdown({
      ...
      options: resolvePreset('full-conversation'),
    });
  ...
  it('renders Files section in timeline mode for with-diffs preset', () => {
    const markdown = prToMarkdown({
      pr: basePr,
      files,
      issueComments,
      ...
      reviews,
      commits,
      options: resolvePreset('with-diffs'),
    });
  ...
  it('does not render Files section in timeline mode when includeFileDiffs is false', () => {
    const markdown = prToMarkdown({
      pr: basePr,
      files,
      issueComments,
